...
class DosenWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.txtKodeDosen.returnPressed.connect(self.search_data) # Jika menekan tombol Enter saat berada di textbox kode_dosen
    ...
    def select_data(self):
        try:
            ...
            self.gridDosen.setHorizontalHeaderLabels(['ID', 'Kode Dosen', 'Nama Lengkap', 'Jenis Kelamin', 'Email'])
    ...
    def search_data(self):
        try:
            kode_dosen=self.txtKodeDosen.text()
            dsn = Dosen()
            # search process
            result = dsn.getByKodeDosen(kode_dosen)
            a = dsn.affected
            if(a!=0):
                self.txtNama.setText(dsn.nama.strip())
                self.cboJk.setCurrentText(dsn.jk.strip())
                self.txtEmail.setText(dsn.email.strip())
    ...
    def save_data(self):
        try:
            ...
            if(self.edit_mode==False):
                ...
                dsn.jk = jk
                dsn.email = email
                a = dsn.updateByKodeDosen(kode_dosen)
                if(a>0):
    ...
    def delete_data(self):
        try:
            ...
            if(self.edit_mode==True):
                a = dsn.deleteByKodeDosen(kode_dosen)
                if(a>0):
